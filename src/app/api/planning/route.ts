...
import { createPlanningMessage, getSession, listPlanningMessages } from '@/lib/db'
import { streamChat, ChatMessage, buildMultimodalContent } from '@/lib/llm'
import { assemblePlanningContext } from '@/lib/context'
...
export async function POST(req: NextRequest) {
  ...
  const { sessionId, message, provider, model, attachments, documentText, imageDescriptionText, imageParsingMethod, documentParsingMethod, embeddingConfig, contextLength, compressionThreshold } = body as {
    sessionId: string; message: string; provider: Provider; model: string
  ...
  if (!sessionId || !message || !provider?.apiKey) {
  ...
  if (!session) {
    return new Response(JSON.stringify({ error: '学习会话不存在，请从首页重新创建' }), {
  ...
  if (embeddingConfig && embeddingConfig.apiKey) {
    try {
      ...
      const results = await retrieveRelevantChunks(message, sessionId, embeddingConfig)
  ...
  const ctx = await assemblePlanningContext(sessionId, retrievedChunks, provider, model, contextLength, compressionThreshold)
